...
// ============================================
// 8. AUTO-APPROVE/REJECT LOGIC
// ============================================

export async function processTaskVerification(
  taskCompletionId: string,
  userId: string,
  fraudCheckResult: FraudCheckResult
): Promise<boolean> {
  if (fraudCheckResult.decision === "auto_approve") {
    // Get task reward points
    const { data: taskCompletion } = await supabase
      .from("task_completions")
      .select("tasks(reward_points, id)")
      .eq("id", taskCompletionId)
      .single();

    if (!taskCompletion?.tasks?.reward_points) return false;

    const rewardPoints = taskCompletion.tasks.reward_points;

    // Get user profile for multiplier
    ...
    const multiplier =
      userProfile.plan === "gold" ? 4 : userProfile.plan === "silver" ? 2 : 1;
    const totalPoints = rewardPoints * multiplier;

    // Update task_completions status
    await supabase
      .from("task_completions")
      .update({ status: "approved" })
      .eq("id", taskCompletionId);

    // Award points and balance
    await supabase
      .from("profiles")
      .update({
        points: (userProfile.points ?? 0) + totalPoints,
        balance: Number(userProfile.balance ?? 0) + totalPoints / 1000,
      })
      .eq("id", userId);

    return true;
  } else if (fraudCheckResult.decision === "auto_reject") {
